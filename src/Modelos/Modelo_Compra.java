...
public class Modelo_Compra {
    ...
    public ResultSet seleccionarProveedores(long codigo) {
        Mysql sql = new Mysql();
        ResultSet rs = null;
        PreparedStatement ps = null;
        try {
            String orden = "select * from proveedor where RUC = ? ";
            ps = sql.conectar().prepareStatement(orden);
            ps.setLong(1, codigo);
            rs = ps.executeQuery();
        } catch (Exception e) {
    ...
    public ResultSet seleccionarRUCProveedores() {
        Mysql sql = new Mysql();
        ResultSet rs = null;
        PreparedStatement ps = null;
        try {
            String orden = "select RUC from proveedor";
            ps = sql.conectar().prepareStatement(orden);
            rs = ps.executeQuery();
        } catch (Exception e) {
            JOptionPane.showMessageDialog(null, "Error al conectarse con la base de datos", "ERROR", 0);
